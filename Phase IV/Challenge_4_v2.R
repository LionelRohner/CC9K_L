...
colnames(nDigits) <- c("Rotations (i.e. # of digits)","# of primes with that rotation")
...
for(rotation in 1:maxRotation){
  ...
  for(potCyclicPrimes in primesRot){
    ...
    # transform prime to character vector
    ...
    if(sort(rotVec) %in% rotVecCache){
      next
    } else {
      # else add character prime to cache
      rotVecCache <- append(rotVecCache, sort(rotVec))
    }
