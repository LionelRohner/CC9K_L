...
checkPrimeRedux3 <- function(numb, exclusio){
  upperBound <- sqrt(numb)
  seqOfDivisors <- 10:upperBound
  seqOfDivisors <- setdiff(seqOfDivisors, exclusio)
  if (any(numb %% seqOfDivisors == 0)){
    return(FALSE)
  } else {
    return(TRUE)
  }
...
primeEvalFaster <- function(from = 0, to, countOnly = TRUE){
  ...
  # get all multiples of 2, 3, 5, 7
  multiplesOf2357 <- c()
  for (i in c(2,3,5,7)){
    n = 0
    while(i^n < round(sqrt(to))){
      multiplesOf2357 <- append(multiplesOf2357, i)
      n = n + 1
    }
  }
  ...
  if(countOnly == TRUE){
    cnt = length(outputPrimes)
    for(numb in vecNat){
      if(checkPrimeRedux3(numb, multiplesOf2357)){
        cnt = cnt + 1
      }
    }
    return(cnt)
